...
export const CONFIG = {
  ...
  // --- 狙い方：手の「向き」で狙う（レーザーポインター／Wiiリモコン方式）---
  // 'orientation' = 手のひらが向いている方向でカーソルを動かす（手を傾けるだけ。
  //                 手の位置は関係ないので、小さい子でも腕を上げずに上の的を狙える）
  // 'position'    = 旧方式。手の「位置」をそのままカーソルにする（手を動かす必要あり）
  AIM_MODE: 'orientation',
  AIM_GAIN: 2.4,            // 傾けたときカーソルが動く量。大きいほど少しの傾きで大きく動く
  AIM_DEADZONE: 0.05,       // 中央付近の遊び。手ぶれで中心がプルプルしないように
  AIM_INVERT_X: false,      // 左右の動きが逆に感じたら true にする
  AIM_INVERT_Y: false,      // 上下の動きが逆に感じたら true にする
  ...
  BULLET_GRAVITY: 4200,     // 弧(ロフト)の高さ。大きいほど高く上がって落ちる（勢いが強く見える）
